...
const char * const BITCOIN_CONF_FILENAME = "iluminum.conf";
const char * const BITCOIN_PID_FILENAME = "iluminumd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "iluminum" is a composite category enabling all Iluminum-related debug output
            if(ptrCategory->count(string("iluminum"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IluminumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IluminumCore
    // Mac: ~/Library/Application Support/IluminumCore
    // Unix: ~/.iluminumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IluminumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/IluminumCore";
#else
    // Unix
    return pathRet / ".iluminumcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty iluminum.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override iluminum.conf
